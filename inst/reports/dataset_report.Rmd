...
```{r, echo=FALSE}
if("CSR_EMBARGO" %in% names(d)) {
  cat("---------------------------------------------------------------\n")
  cat("EMBARGO FLAG IS SET--DATA NOT TO BE RELEASED\n")
  cat("---------------------------------------------------------------\n")
}
```
...
compute_interval <- function(dsd) {
  if(is.null(dsd)) {
    return(tibble(First_obs = NA, Last_OBS = NA, `Interval (m)` = NA))
  } else {
    dsd %>% 

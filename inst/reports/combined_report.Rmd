...
x <- desc[c("CSR_DATASET", "Records")]
knitr::kable(cbind(x, years), format.args = list(big.mark = ',')) %>% 
...
do.call("rbind", c(coverage, make.row.names = FALSE)) %>% 
  ...
  summarise(N = n()) %>% 
  ungroup %>% 
  mutate(ym = lubridate::ymd(paste(Year, Month, "1"))) %>% 
  ggplot(aes(ym, CSR_DATASET, fill = N)) + geom_tile()
```

...
# TODO: this duplicates code in dataset_report.Rmd
compute_interval <- function(dsd) {
  dsd %>% 
    mutate(Year = year(CSR_TIMESTAMP)) %>% 
    arrange(CSR_TIMESTAMP, CSR_PORT) %>% 
    group_by(Year, CSR_PORT) %>% 
    summarise(First_obs = min(CSR_TIMESTAMP),
              Last_obs = max(CSR_TIMESTAMP),
              N = length(CSR_TIMESTAMP),
              `Interval (m)` = median(as.numeric(difftime(CSR_TIMESTAMP, lag(CSR_TIMESTAMP), units = "mins")), na.rm = TRUE),
...
years <- lapply(all_data, function(x) {
  if(is.data.frame(x$data) & "CSR_TIMESTAMP" %in% names(x$data)) {
    ...
    data.frame(Start = NA, End = NA, `Interval (m)` = NA,
...
```{r database-growth}
system.file(file.path("extdata/database_growth.csv"),
            package = "cosore", mustWork = TRUE) %>% 
  readr::read_csv(col_types = "Ddddd") %>% 
  gather(Variable, Value, Records, Size_Mb, Datasets) %>% 

...
le <- lapply(all_data, function(x) {
  if(is.data.frame(x$data) & "CSR_TIMESTAMP" %in% names(x$data)) {
    data.frame(CSR_DATASET = x$description$CSR_DATASET,
               CSR_LATITUDE = x$description$CSR_LATITUDE,
               CSR_ELEVATION = x$description$CSR_ELEVATION,
               CSR_IGBP = x$description$CSR_IGBP,
...
p <- ggplot(le, aes(CSR_LATITUDE, CSR_FLUX, color = CSR_ELEVATION)) + 
  geom_jitter() + facet_wrap(~CSR_IGBP) +

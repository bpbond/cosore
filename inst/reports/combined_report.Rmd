...
compute_years <- function(x) {
  if(is.data.frame(x$data) & "CSR_TIMESTAMP_BEGIN" %in% names(x$data)) {
    co2 <- "CSR_FLUX_CO2" %in% names(x$data) && any(!is.na(x$data$CSR_FLUX_CO2))
    ch4 <- "CSR_FLUX_CH4" %in% names(x$data) && any(!is.na(x$data$CSR_FLUX_CH4))
    gases <- paste(c("CO2", "CH4")[c(co2, ch4)], collapse = ", ")
    
    tibble(Dataset = x$description$CSR_DATASET,
           Records = x$diagnostics$CSR_RECORDS,
           Start = min(x$data$CSR_TIMESTAMP_BEGIN, na.rm = TRUE),
           End = max(x$data$CSR_TIMESTAMP_BEGIN, na.rm = TRUE),
    ...
    tibble(Dataset = x$description$CSR_DATASET,
           Records = 0,
           Start = NA, End = NA, `Interval (min)` = NA,
...
x <- x[x$CSR_FLUX_CO2 > -1 & x$CSR_FLUX_CO2 < 20 & is.finite(x$CSR_FLUX_CO2),]
ggplot(x, aes(x = CSR_FLUX_CO2)) + 

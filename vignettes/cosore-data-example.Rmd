...
```{r, include=FALSE}
if(travis) {
  db_info <- tibble::tibble(CSR_IGBP = "Deciduous broadleaf forest",
                            CSR_DATASET = "d20190415_VARNER")  
} else {
...
```{r, include=FALSE}
# We need to do this for now because no data available when on Travis
if(travis) {
  sr <- data.frame(CSR_TIMESTAMP_BEGIN = 1, CSR_FLUX = 1, CSR_PORT = 1, CSR_T5 = 1)
} else {
  sr <- varner$data
}
nrow(sr)
...
if(travis) {
  tdf <- data.frame(CSR_DATASET = "d20190415_VARNER",
                    CSR_SITE_NAME = 1, CSR_LONGITUDE = 1, CSR_LATITUDE = 1)
} else {
...
```{r, include=FALSE}
if(travis) {
  tdf <- data.frame(CSR_TIMESTAMP_BEGIN = ymd_hm("2020-01-31 7:01"), 
                    CSR_FLUX = 1, CSR_DATASET = "d20190415_VARNER")
} else {

...
to install COSORE: `devtools::install_github("bpbond/cosore")`.
...
The `cosore` R package provides a useful function that gives an overview of the entire database:
...
There's lots of information, including dataset name; geographic location; number of records; vegetation types; and gases, fluxes, and dates measured. (Note that much of this is also summarized in the `Report-all.html` file included with the data in each [release](https://github.com/bpbond/cosore/releases).)
...
We also want to look at the actual data:
...
Something odd is going on with the November data; we might want to investigate further.
...
Finally, we can use the `description` table information to get a full citation (which you will **definitely** want to use in our published analysis, right?):
...
tdf_combined$Year <- year(tdf_combined$CSR_TIMESTAMP_BEGIN)

# for each month, compute mean flux for each hour of the day
tdf_smry <- aggregate(CSR_FLUX_CO2 ~ CSR_DATASET + CSR_LATITUDE + Year + Month + Hour,
                      FUN = mean, data = tdf_combined)

ggplot(tdf_smry, aes(Hour, CSR_FLUX_CO2, color = CSR_LATITUDE)) + 
  geom_point(size = 0.5) + facet_wrap(~Month) +
  ggtitle("Hourly flux, by month of year")
```

Note that in COSORE all timestamps are in the site's _local_, _standard_ time.

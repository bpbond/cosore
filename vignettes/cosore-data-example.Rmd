...
data_loaded <- exists("cosore") && is.data.frame(cosore$d20190415_VARNER$data)
  ...
  cosore <- list(
    d20190415_VARNER = list(
      description = data.frame(CSR_DATASET = "d20190415_VARNER",
                               ...
                               CSR_LONGITUDE = 1,
                               CSR_LATITUDE = 1),
      data = data.frame(CSR_TIMESTAMP_BEGIN = lubridate::ymd_hm("2019-06-05 07:12"),
                        CSR_FLUX = 1,
                        CSR_PORT = 1,
                        CSR_T5 = 1))
  )
...
This database is comprised of a collection of _datasets_, each converted to a standard format and units. A dataset is one or more files of continuous (automated) soil respiration data, with accompanying metadata, with all measurements taken at a single site and with constant treatment assignments.
...
tdf_dat %>% 
  ggplot(aes(CSR_TIMESTAMP_BEGIN, CSR_FLUX, color = CSR_DATASET)) + 
  geom_point(size = 0.5) + 
...
tdf_dat %>% 
  ...
  # add some new fields
  mutate(Hour = hour(CSR_TIMESTAMP_BEGIN),
...
# Using the _database_ versus using the _package_

Note that it's not necessary to install the `cosore` package to use the 
COSORE database. The former is composed of software tools used to parse dataset 
metadata, read and normalize raw data, and generate the database, while latter is
a collection of files available for download via the package's
[Releases](https://github.com/bpbond/cosore/releases) page.

The package does include a few functions that might be handy for users:

* `csr_table()`, the code for which is provided near the top of this vignette.
* `run_single_report()`, to generate an html report on a single dataset. A  `run_all_reports()` function will generate reports for all datasets.
* `run_combined_report()` provides a summary report, included in the release
download, on the entire database.

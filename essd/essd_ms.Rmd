...
**Figure 3.** Flux observations, by IGBP (cf. Table 1), over time. Each square represents `r format(scaling_factor, big.mark = ",")` observations.
...
Figure 6. Distribution of CO2 fluxes in COSORE datasets, by IGBP classification (cf. Table 1). For visual clarity this figure excludes fluxes <`r min(co2_limits)` and >`r max(co2_limits)` µmol m-2 s-1 (`r format(nrow(out_co2), big.mark = ",")` observations, `r pct_co2`% of the data).
...
Figure 7. Distribution of CH4 fluxes in COSORE datasets, by IGBP classification (cf. Table 1). For visual clarity this figure excludes fluxes <`r min(ch4_limits)` and >`r max(ch4_limits)` nmol m-2 s-1 (`r format(nrow(out_ch4), big.mark = ",")` observations, `r pct_ch4`% of the data).

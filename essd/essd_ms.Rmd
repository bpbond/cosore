...
```{r flux-density, message = FALSE, echo = FALSE}
library(waffle)
...
```{r lat-coverage, message = FALSE, echo = FALSE}

data_meta %>% 
  select(CSR_DATASET, CSR_LATITUDE, CSR_TIMESTAMP_BEGIN, CSR_TIMESTAMP_END, CSR_IGBP) %>% 
  group_by(CSR_DATASET, CSR_IGBP) %>% 
  summarise(Latitude = mean(CSR_LATITUDE), Timestamp_begin = min(CSR_TIMESTAMP_BEGIN), Timestamp_end = max(CSR_TIMESTAMP_END), n = n()) -> timestamp_lengths
                                       
lat_intervals <- as.character(cut_interval(timestamp_lengths$Latitude, length = 5))

ggplot(timestamp_lengths, aes()) + 
  geom_segment(aes(x = Timestamp_begin, y = lat_intervals, xend = Timestamp_end, yend = lat_intervals),
               color = "blue4", size = 2) + 

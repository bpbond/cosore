...
results <- lapply(datasets, function(x) {
  ...
  if(!"CSR_FLUX_CO2" %in% names(d$data)) d$data$CSR_FLUX_CO2 <- NA
  if(!"CSR_FLUX_CH4" %in% names(d$data)) d$data$CSR_FLUX_CH4 <- NA
  d$data %>% 
...
**Figure 4.** Temporal data coverage by day of year and hemisphere

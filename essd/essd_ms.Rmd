...
It is targeted at continuous soil respiration flux data--i.e., those measured by automated systems on a e.g. half hourly basis (CITATION TODO)--but its design accomodates survey-style R~S~ fluxes, methane fluxes, and chamber measurements of net ecosystem exchange as well. 
Its development started in April 2019, and as of this writing (`r Sys.Date()`) the COSORE version number is `r db_vers`. 
The deatabase currently has `r nrow(db)` contributed datasets with a total of `r db_records` million flux observations across `r db_years` years and five continents (Table X, Figure X).
...
The database license is CC-BY-4 (https://creativecommons.org/licenses/by/4.0/). This is identical to that used by e.g. Ameriflux and FLUXNET Tier 1. 
...
Papers or other research products using ISRaD should cite this publication. 
...
flat-file CSV (comma-separated value), readable by any modern computing system, and are 
...
* `csr_dataset()` returns a single dataset: an R list structure, with each element a table (`description`, `contributors`, etc., as described above). 
...
All user questions and contributions are subject to a code of conduct (TODO).
...
```{r worldmap}
# We want a satellite image with COSORE points overlayed

ggplot(data = world, aes(x = long, y = lat)) + 
  geom_polygon(aes(group = group), fill = "white", color = "grey") + 
  coord_map(xlim=c(-180,180)) + 
  theme_minimal() + 
  geom_point(data = db, aes(x = CSR_LONGITUDE, y = CSR_LATITUDE))

#what about coloring by no of records? a sort of heatmap of COSORE coverage

bbox <- make_bbox(lon = c(-170, 170), lat = c(-70,70), f = 0)
map <- get_map(location = bbox, source = "google", maptype = "satellite", zoom = 1)

ggmap(map) + 
  geom_point(data = db, aes(x = CSR_LONGITUDE, y = CSR_LATITUDE), alpha = 0.4, color = "red", size = 2) + 
  coord_fixed(xlim= c(-160,160), ylim= c(-75,75)) +
# xlim(c(-160, 160)) + 
#  ylim(c(-75,75)) +
  labs(x = "Longitude", y = "Latitude")

# Third option (best)
mapWorld <- borders("world", colour="grey85", fill="grey95")

ggplot() + 
  mapWorld + 
  theme(panel.background = element_rect(fill = "slategray1",
                                    colour = "slategray1"), panel.grid = element_blank(),
        legend.position="bottom") + 
   geom_point(data = db, aes(x = CSR_LONGITUDE, y = CSR_LATITUDE, fill = "COSORE Submission Site"), 
             alpha = 0.5, color = "#FC4E07", size = 4, shape = 18) +
  labs(x = "Longitude", y = "Latitude") +
#  scale_color_discrete(values="#FC4E07",
#                     labels = "COSORE Submissions")
...
ggplot(data_meta, aes(x = CSR_IGBP))

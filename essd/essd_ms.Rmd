...
* Extremely low or high (<-10 and >50 µmol CO~2~ m^-2^ s^-1^ respectively; currently there are no CH~4~ limits) flux rates
...
csr_table("diagnostics", quiet = TRUE) %>% 
  ...
  summarise(removed = CSR_RECORDS_REMOVED_NA + CSR_RECORDS_REMOVED_ERR +
              CSR_REMOVED_LOW_CO2 + CSR_REMOVED_HIGH_CO2 +
              CSR_REMOVED_LOW_CH4 + CSR_REMOVED_HIGH_CH4 +
...
heterotrophic respiration fluxes. There currently is no standardized template data contributors must follow, but we anticipate this changing before version 1.0 (planned for late 2020).
...
# Note that the following code assumes you have called `ggmap::register_google()` already

bbox <- make_bbox(lon = c(-160, 160), lat = c(-70,70)) # make a coordinate box 
...
ggmap(map) + 
  geom_point(data = db, aes(x = CSR_LONGITUDE, y = CSR_LATITUDE, 
                            fill = "COSORE Submission Site", size = CSR_RECORDS), 
...
#need to find and resolve removed datapoint throwing warning
```

**Figure 2.** Waffle plot of COSORE Coverage by IGBP by year of measurement. Here, one square equals 1,000 datapoints.

```{r flux-density, message = FALSE, echo = FALSE, eval=FALSE}
library(waffle)

# SP I set this chunk to do-not-run because it requires loading the entire `data` table, 
# which is a killer (hundreds of MB in size and steady getting bigger). Let's rewrite.
# Can you just get the information you need from `csr_database()`?
...
data_meta %>% 
  ...
  summarise(CSR_FLUX_CO2 = mean(CSR_FLUX_CO2, na.rm = TRUE), Date = mean(Date), n = n()/1000) -> avg
...
**Figure 3.** Temporal data coverage by latitude.

```{r lat-coverage, message = FALSE, echo = FALSE, eval=FALSE}

data_meta %>% 
  select(CSR_DATASET, CSR_LATITUDE, CSR_TIMESTAMP_BEGIN, CSR_TIMESTAMP_END, CSR_IGBP) %>% 
  group_by(CSR_DATASET, CSR_IGBP) %>% 
  summarise(Latitude = mean(CSR_LATITUDE), 
            Timestamp_begin = min(CSR_TIMESTAMP_BEGIN), 
            Timestamp_end = max(CSR_TIMESTAMP_END), n = n()) -> 
  timestamp_lengths

lat_intervals <- as.character(cut_interval(timestamp_lengths$Latitude, length = 5))

ggplot(timestamp_lengths, aes()) + 
  geom_segment(aes(x = Timestamp_begin, y = lat_intervals, 
                   xend = Timestamp_end, yend = lat_intervals),
               color = "blue4", size = 2) + 
  theme_minimal() + labs(x = "Timestamp", y = "Latitude Interval")

#still need to reorder intervals to [-35,30] is at the bottom of the plot
```

```{r doy-counts}
data_meta %>% 
  mutate(DOY = yday(CSR_TIMESTAMP_BEGIN),
         Hemi = ifelse(CSR_LATITUDE < 0, "Southern Hemisphere", "Northern Hemisphere")) %>% 
  group_by(Hemi, DOY) %>% 
  summarise(CSR_FLUX_CO2 = mean(CSR_FLUX_CO2, na.rm = TRUE), n = n()) %>% 
  filter(DOY <= 365 ) -> doy_counts

ggplot(data = doy_counts, aes(x = DOY, y = n)) + 
  geom_line(color = "blue4", size = 1) + 
  facet_wrap(~Hemi, ncol = 1, scales = "free_y") +
  labs(x= "Day of Year", y = "Number of Points") +
  theme_minimal()
```

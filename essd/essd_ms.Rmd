...
The interval between measurements ranges from `r round(min(intervals$Interval, na.rm = TRUE), 0)` to `r round(max(intervals$Interval, na.rm = TRUE), 0)` minutes, with 25%-50%-75% quantile values of `r q[2]`, `r q[3]`, and `r q[4]` minutes respectively. A one-hour interval between measurements is thus by far the most common choice.
...
**Figure 4.** Flux observations, by IGBP (cf. Table 1), over time. Each square represents `r format(scaling_factor, big.mark = ",")` observations.
...
**Figure 5.** Temporal data coverage by latitude.
...
p <- results_lat %>% 
  ...
  geom_point(size = 5) + 
  scale_color_distiller(palette = "YlGnBu", name = "Observations", labels = comma) +
...
Figure 8. Distribution of CH4 fluxes in COSORE datasets, by IGBP classification (cf. Table 1). For visual clarity this figure excludes fluxes <`r min(ch4_limits)` and >`r max(ch4_limits)` nmol m-2 s-1 (`r format(nrow(out_ch4), big.mark = ",")` observations, `r pct_ch4`% of the data).

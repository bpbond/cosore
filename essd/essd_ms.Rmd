...
# This first option is ideal, but Google will only give square tiles

# Second option: a "drawn" map 
...
# This is just an idea, needs major tweaks
ggplot(data_meta, aes(x = CSR_IGBP)) + 
  geom_density() + 
  theme(axis.text.x = element_text(angle=90))

data_meta %>% mutate(Date = date(CSR_TIMESTAMP_BEGIN), Month = month(CSR_TIMESTAMP_BEGIN), Year = year(CSR_TIMESTAMP_BEGIN)) %>% 
  group_by(Year, CSR_IGBP) %>%
  summarise(CSR_FLUX = mean(CSR_FLUX, na.rm = TRUE), Date = mean(Date), n = n()) -> avg

# tile plot by IGBP
ggplot(avg, aes(x = Date, y = CSR_IGBP, color = n)) + geom_tile(size = 2)

#box plot by IGBP
ggplot(avg, aes(x = Date, y = CSR_IGBP, group = CSR_IGBP)) + geom_violin() + theme_minimal()

# waffle plot no of records by IGBP
ggplot(avg_clean, aes(fill = CSR_IGBP, values = n)) +
        geom_waffle(color = "white", size = .25, n_rows = 10, flip = TRUE) +
        facet_wrap(~Year, nrow = 2, strip.position = "bottom") +  coord_equal() +
     labs(title = "Faceted Waffle Bar Chart",
        subtitle = "annual COSORE coverage by IGBP",
        x = "Year",
        y = "Count") +
     theme_minimal() +

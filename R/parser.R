...
#' @examples
#' extract_line(c("1:one", "2:two"), "two")
#' extract_line(c("one:1", "two:2"), "two", numeric_data = TRUE)
...
#' @examples
#' resolve_dataset("d20190406_TEST")
...
#' @examples
#' read_file("d20190406_TEST", "DESCRIPTION.txt")
read_file <- function(dataset_name, file_name, comment_char = "#") {
...
#' @examples
#' read_description_file("d20190406_TEST")
read_description_file <- function(dataset_name) {
  ...
  data.frame(Dataset = dataset_name,
             ...
             Primary_pub = extract_line(file_data, "Primary publication", required = FALSE),
...
#' @examples
#' read_contributors_file("d20190406_TEST")
